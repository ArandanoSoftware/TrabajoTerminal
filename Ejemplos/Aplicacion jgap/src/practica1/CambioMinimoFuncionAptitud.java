...
public class CambioMinimoFuncionAptitud extends FitnessFunction{
	...
	public CambioMinimoFuncionAptitud(int monto) {
        ...
        //      canbio para moneda ecuatoriana
		}
		montoObjetivo = monto;
	}
	...
	*
	*/
        // ****** cambio para moneda ecuatoriana
	public static int montoCambioMoneda(IChromosome cromosoma) {
		int unDolar = getNumeroDeComendasDeGen(cromosoma, 0);
		int cincuentaCentavos = getNumeroDeComendasDeGen(cromosoma, 1);
		int veinticincoCentavos = getNumeroDeComendasDeGen(cromosoma, 2);
		int diezCentavos = getNumeroDeComendasDeGen(cromosoma, 3);
		int cincoCentavos = getNumeroDeComendasDeGen(cromosoma, 4);
                int unCentavo = getNumeroDeComendasDeGen(cromosoma, 5);

		return (unDolar * 100) + (cincuentaCentavos * 50) + (veinticincoCentavos * 25) + (diezCentavos * 10) + (cincoCentavos * 5) + unCentavo;
	}
